...
class Score {
  ...
  update(deltaTime) {
    ...
    this.score += deltaTime * 0.001; // 점수는 deltaTime의 0.001배만큼 증가

    // 점수가 100에 도달하고, stageChange가 true일 때 스테이지 전환 이벤트 발생
    if (Math.floor(this.score) === 100 && this.stageChange) {
      this.stageChange = false; // 스테이지 전환은 한 번만 이루어지도록 stageChange를 false로 설정
      sendEvent(11, { currentStage: 1000, targetStage: 1001 }); // 스테이지 전환 이벤트를 보냄
    }
  }
